...
{
    internal class CloneToCommand : Command, IItemBucketsCommand
    {
        public override void Execute(CommandContext context)
        {
            ...
            if (context.Items.Length > 0)
            {
                ...
                {
                    Log.Error("Clone Items - Unable to cast current item - " + context.Items[0].GetType().FullName, this);
                    return;
                }
                using (IProviderSearchContext providerSearchContext = ContentSearchManager.GetIndex(sitecoreIndexableItem).CreateSearchContext(SearchSecurityOptions.Default))
                {
                    ...
                    Items.CloneTo(list.Distinct(new IDComparer()).ToArray());
                    //
                }
            }
        }
